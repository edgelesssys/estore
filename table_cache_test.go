...
func TestTableCacheErrorBadMagicNumber(t *testing.T) {
	...
	require.Equal(t,
		"pebble: backing file 000003 error: cipher: message authentication failed",
		err.Error())
}
...
func TestTableCacheNoSuchFileError(t *testing.T) {
	...
	require.Regexp(t, "directory contains [67] files, 1 unknown, 0 tables, [12] logs, 1 manifests", logger.fatalMsgs[0]) // EDG: as we don't recycle logs, one may already be deleted
}
